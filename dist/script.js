...
const POINTS = [
  { id:"newborn-face-bias", short:"Newborn face bias", label:"Newborn face preference", family:"human", prior:"built",
    ...
    notTo:"Anything; and it fades within months as the brain's dedicated face system takes over",
  ...
  { id:"infant-segmentation", short:"Infant segmentation", label:"Infant statistical learning", family:"human", prior:"built",
    ...
    notTo:"Open question; how far this pattern-tracking ability reaches is still being researched",
  ...
  { id:"toddler-fast-mapping", short:"Fast mapping", label:"Toddler fast mapping", family:"human", prior:"built",
    ...
    notTo:"Categories outside language; it leans on word-learning assumptions the child already has",
  ...
  { id:"child-second-language", short:"Child L2", label:"Child second-language learning", family:"human", prior:"built",
    ...
    notTo:"Still one language — the child's advantage is in how well they end up, not in breadth",
  ...
  { id:"omniglot-human", short:"Human one-shot", label:"Adult one-shot character learning", family:"human", prior:"built",
    ...
    unit:"one example of a character", needed:"1",
  ...
  { id:"learning-to-drive", short:"Learning to drive", label:"Learning to drive", family:"human", prior:"built",
    ...
    to:"Unfamiliar cars, unfamiliar cities, roads never seen before — genuine generalisation, but all of it inside driving",
  ...
  { id:"one-shot-faces", short:"One-shot faces", label:"Recognising a face after one meeting", family:"human", prior:"built",
    ...
    notTo:"Unfamiliar faces from a single encounter — people routinely judge two photographs of the same unfamiliar person to be two different people. People are also reliably somewhat worse with faces from ethnic groups other than their own",
  ...
  { id:"chess-expertise", short:"Chess expertise", label:"Human chess expertise", family:"human", prior:"built",
    ...
    note:"A useful corrective: the spread on this map exists inside a single species, so it is not a story about humans beating machines. Two details to get right. The ten-year figure is Simon and Chase's rule, not Ericsson's violin study. And the practice account is contested — one review of the studies finds practice accounts for only about a quarter of the differences between players. The part this point rests on, that chess skill goes nowhere else, is the solid part." },
  ...
  { id:"honeybee-sameness", short:"Honeybee same/diff", label:"Honeybee same/different concept", family:"animal", prior:"built",
    ...
    to:"New patterns, and across senses — a rule learned by sight carried over to smell",
  ...
  { id:"taste-aversion", short:"Taste aversion", label:"Conditioned taste aversion", family:"animal", prior:"built",
    ...
    lifetime:{e:10,t:0}, total:{e:1,t:0},
    ecological:"Very well. Taste paired with illness is exactly the pairing this animal evolved to detect, which is why one trial is enough.",
  ...
  { id:"pavlov-dogs", short:"Pavlov's dogs", label:"Pavlovian conditioning", family:"animal", prior:"built",
    ...
    notTo:"Anything that needs the link to be understood rather than just registered",
  ...
  { id:"crow-trap-tube", short:"Crow trap-tube", label:"New Caledonian crow, trap-tube", family:"animal", prior:"built",
    ...
    to:"Physical problems of a similar shape",
    notTo:"Contested — whether crows understand cause and effect, or just learn which end looks right, is actively disputed",
    lifetime:{e:4,t:3}, total:{e:2,t:3},
    source:"Taylor et al. 2009", confidence:"low",
    note:"Whether crows understand the physics or have learned which pictures predict food is genuinely disputed, which is why this point carries a low-confidence ring rather than a confident placement. On how a small brain manages it at all: crow and parrot forebrains pack neurons very densely, so brain volume is the wrong thing to measure." },
  ...
  { id:"rats-beat-humans", short:"Rats beat humans", label:"Rats outperforming humans at generalisation", family:"animal", prior:"built",
    ...
    to:"New examples of the same category — and better than the humans tested on the same task",
  ...
  { id:"alphazero", short:"AlphaZero", label:"AlphaZero (chess)", family:"classic_ai", prior:"learned",
    ...
    note:"Superhuman skill, and almost no intelligence by Chollet's definition, which counts skill at new tasks rather than skill at one — a useful provocation. It also shows something this map's transfer axis cannot. When DeepMind's earlier DQN system learned 49 Atari games, the same design and settings worked for all of them, but a separate copy had to be trained for each game on 50 million frames, roughly 38 days of play. The design carried over completely; what each copy had learned carried over not at all. This axis only measures the second." },
  ...
  { id:"deep-blue", short:"Deep Blue", label:"Deep Blue", family:"classic_ai", prior:"built",
    ...
    what:"The machine that beat Kasparov in 1997 searched around 200 million positions a second, scoring them with a hand-built formula whose ingredients were written by chess experts and whose weights were tuned against a database of master games, and whose opening book was drawn from over 700,000 grandmaster games.",
    unit:"one grandmaster game used for tuning or stored in the opening book", needed:"over 700,000 grandmaster games, plus hand-written scoring rules",
    to:"Nothing",
  ...
  { id:"mnist", short:"MNIST", label:"MNIST digit classifier", family:"classic_ai", prior:"learned",
    ...
    what:"Sixty thousand labelled images of handwritten digits, used to train a network to tell 0 through 9 apart. Modern networks get over 99% of the test images — ones held back from training — right.",
    ...
    note:"The 'hello world' of machine learning, and a clean contrast with the one-shot character points to its right. Sixty thousand examples for ten categories, against one example per category for a person or for Bayesian Program Learning. Same kind of task, ten thousand times apart." },

  { id:"alexnet", short:"AlexNet", label:"AlexNet / ImageNet", family:"classic_ai", prior:"learned",
    task:"Large-scale image classification",
    what:"A network trained on 1.2 million labelled photographs across a thousand categories, which halved the error rate on the ImageNet benchmark in 2012. What it learned turned out to be partly reusable: the general shape and texture detectors it builds early help on other image tasks, the later and more specific ones do not.",
    unit:"one labelled image", needed:"over a million",
    to:"Other image tasks, by reusing what it already learned — the origin of transfer learning as standard practice",
    notTo:"Non-visual domains",
    lifetime:{e:1,t:4}, total:{e:1,t:4},
    source:"Krizhevsky, Sutskever & Hinton 2012; transfer analysed in Yosinski et al. 2014", confidence:"high",
    note:"Transfer is not all-or-nothing. The general detectors the network builds first move to new image tasks; the specialised ones it builds later do not." },
  ...
  { id:"bayesian-program-learning", short:"Bayesian programs", label:"Bayesian Program Learning", family:"classic_ai", prior:"built",
    ...
    what:"The model treats a character as a set of pen strokes and rules for joining them. Shown one example of a character it has never seen, it works out the strokes that would have drawn it, then produces new versions. Asked to pick out other examples of a character it had seen once, it was wrong on 3.3% of trials, against 4.5% for people.",
    unit:"one example of a character", needed:"1",
    to:"New alphabets, as long as they are built out of pen strokes the same way",
    notTo:"Anything outside handwritten characters — the machinery was designed for that one job",
    lifetime:{e:9,t:2}, total:{e:3,t:2},
    source:"Lake, Salakhutdinov & Tenenbaum 2015, Science", confidence:"high", keystone:true,
    note:"**The clearest case on the map.** Shown a character once, it matched human accuracy — 3.3% error against a human 4.5% — from a hand-designed story about strokes, sub-strokes and how they join. Be precise about what was designed: the structure was, while the basic stroke shapes were fit to 30 other alphabets, so 'entirely hand-built' overstates it. Even so, this is the one point where you can read the domain assumption straight off the model's definition." },
  ...
  { id:"act-r", short:"ACT-R", label:"ACT-R cognitive model", family:"classic_ai", prior:"built",
    ...
    what:"A model of human thinking in which a researcher writes out the knowledge and rules for a task by hand, after which the model predicts how long people take and which mistakes they make. Every new subject has to be written out by hand first.",
    unit:"one training instance", needed:"few, given hand-built task knowledge",
    to:"Nothing automatically — each new subject is written out by hand",
    notTo:"Any subject nobody has written out by hand",
  ...
  { id:"llm-pretraining", short:"LLM pretraining", label:"LLM pretraining", family:"current_ai", prior:"learned",
    ...
    to:"An unusually wide range of later tasks it was never trained for",
  ...
  { id:"llm-in-context", short:"LLM in-context", label:"LLM in-context learning", family:"current_ai", prior:"learned",
    ...
    what:"The same pretrained model is shown a few examples of a task inside the prompt and does it immediately — no training and no change to the model itself. The task can be one nobody designed it for.",
  ...
  { id:"alphafold", short:"AlphaFold", label:"AlphaFold", family:"current_ai", prior:"learned",
    ...
    unit:"one structure in the training set", needed:"roughly 170,000 experimental structures, plus about 350,000 of its own predictions fed back in as training data",
  ...
  { id:"recommenders", short:"Recommenders", label:"Recommender systems", family:"current_ai", prior:"learned",
    ...
    note:"The AI most people touch every day, sitting in the same corner as AlphaZero. The systems with the biggest real-world footprint are mostly down here — and cold start, the most expensive open problem in the industry, is this map's efficiency axis asked in the real world." },
  ...
  { id:"arc-agi", short:"ARC-AGI", label:"ARC-AGI", family:"current_ai", prior:"learned",
    ...
    what:"A test made of puzzles in which each task shows two or three worked examples of some transformation on a grid of coloured squares, and the solver has to apply it to a new grid. Every task is different, and they are built so that having seen similar puzzles does not help.",
    unit:"two or three demonstration pairs per task", needed:"2–3",
    to:"By design, each task is new — the test measures learning itself",
    ...
    note:"A target region rather than a data point: it marks where the corner would be if something reached it. There is no honest total-accounting position for a target nobody has reached, so it drops out when the accounting changes. The leaderboard numbers below are contested, and the disagreement between them says more than any single figure." }
];
...
axYhi.textContent = "works in many areas";
